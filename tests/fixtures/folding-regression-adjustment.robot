...
T001 Synthetic Balance Adjustment Scenario
    ...
    ...    status=Completed
    ...    balance=0,00 EUR
    ...    balanceHover=Balanced
    ...    row=1

    Demo UI Navigate To Edit
    Demo UI Edit Verify Visible     visible=Yes
    #> - A manual adjustment is selected even though the case is already balanced.
    Demo UI Edit Enter
    ...    activity=Manual ledger adjustment
    ...    reason=Manual adjustment

    #>> -> The UI rejects an adjustment when the account is already balanced.
    Demo UI Edit Verify Validation
    ...    amount=Yes
    ...    message=Manual adjustment is not allowed for an already balanced account.
    ...
    Demo UI Edit Verify Validation
    ...    amount=Yes
    ...    message=Reverse manual adjustment is not allowed for an already balanced account.

    Demo UI Edit Cancel
    Demo UI Edit Verify Visible     visible=No
